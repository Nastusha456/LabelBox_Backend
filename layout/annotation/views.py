...
from .serializer import AnnClassSerializer, AnnGroupSerializer, AnnLabelSerializer, AnnotationObjectSerializer
...
#class AnnotationObjectView(generics.ListCreateAPIView):
#    serializer_class = AnnotationObjectSerializer
#
#    def get_queryset(self):
#        user_id = self.kwargs.get('user_id')
#        annotation_id_object = self.kwargs.get("pk")
#        if user_id:
#            return AnnotationObject.objects.filter(user=user_id, annotation_id=annotation_id_object)
#        return AnnotationObject.objects.none()
#
#    def list(self, request, *args, **kwargs):
#        queryset = self.get_queryset()
